// Vercel Edge Runtime — Gemini 프록시 (Smile Vision AI SaaS)
// Cold Start <100ms, 본사 GEMINI_API_KEY 서버 보관
//
// ① Auth (Bearer or user_email)  ② Rate Limit (tier)  ③ 호출 로그
// ④ 모드 — text / image (gemini-2.5-flash-image)
...
export default async function handler(req) {
  ...
  catch { return json({ error: '잘못된 JSON' }, 400); }

  const { prompt, imageBase64, model, mode, user_email, engine } = body || {};
  if (!prompt || typeof prompt !== 'string') {
    return json({ error: 'prompt 필드가 필요합니다' }, 400);
  }
  const MODEL = (model && ALLOWED_MODELS.has(model)) ? model : DEFAULT_MODEL;
  const isImageMode = MODEL.includes('image') || mode === 'image';
  const t0 = Date.now();
  ...
  try {
    ...
    if (isImageMode) payload.generationConfig = { responseModalities: ['TEXT', 'IMAGE'] };
    ...
    if (!r.ok) {
      ...
      for (const p of resParts) {
        ...
        if (inl && (inl.mimeType || inl.mime_type || '').startsWith('image/')) {
          ...
          imageDataUrl = `data:${mime};base64,${inl.data}`;
        }
      }
      if (isImageMode && !imageDataUrl) {
        errMsg = '이미지가 생성되지 않았습니다';
        statusCode = 502;
      }
    }
  } catch (e) {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/api_call_logs`, {
        ...
        body: JSON.stringify({
          ...
          metadata: { engine: engine || 'generic', mode: isImageMode ? 'image' : 'text', error: errMsg || null }
        })
      });
    } catch {}
  }
